...
import net.gyula.wildaside.procedures.NaturalSporeblasterStr1AdditionalGenerationConditionProcedure;
import net.gyula.wildaside.init.WildasideModBlocks;

import java.util.Set;
import java.util.List;

public class NaturalSporeblasterStr20Feature extends Feature<NoneFeatureConfiguration> {
	public static NaturalSporeblasterStr20Feature FEATURE = null;
	public static Holder<ConfiguredFeature<NoneFeatureConfiguration, ?>> CONFIGURED_FEATURE = null;
	public static Holder<PlacedFeature> PLACED_FEATURE = null;

	public static Feature<?> feature() {
		FEATURE = new NaturalSporeblasterStr20Feature();
		CONFIGURED_FEATURE = FeatureUtils.register("wildaside:natural_sporeblaster_str_20", FEATURE, FeatureConfiguration.NONE);
		PLACED_FEATURE = PlacementUtils.register("wildaside:natural_sporeblaster_str_20", CONFIGURED_FEATURE, List.of());
		return FEATURE;
	}
	...
	public NaturalSporeblasterStr20Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("wildaside", "natural_spore_blaster"));
		if (template == null)
			return false;
		...
		if ((context.random().nextInt(1000000) + 1) <= 1000000) {
			...
			for (int a = 0; a < count; a++) {
				...
				int y = spawnTo.getY();
				int z = spawnTo.getZ();
				if (!NaturalSporeblasterStr1AdditionalGenerationConditionProcedure.execute(world, x, y, z))
					continue;
