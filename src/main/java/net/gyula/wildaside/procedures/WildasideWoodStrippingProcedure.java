...
public class WildasideWoodStrippingProcedure {
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, Entity entity) {
		if (entity == null)
			return;
		if ((world.getBlockState(new BlockPos(x, y, z))).is(BlockTags.create(new ResourceLocation("wildaside:strippable_wildaside_logs")))) {
			if ((entity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem() instanceof AxeItem) {
				...
				if (world instanceof Level _level) {
					if (!_level.isClientSide()) {
						_level.playSound(null, new BlockPos(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("item.axe.strip")), SoundSource.BLOCKS, 1, 1);
					} else {
				...
				if ((world.getBlockState(new BlockPos(x, y, z))).getBlock() == WildasideModBlocks.BALD_CYPRESS_LOG.get()) {
					{
						BlockPos _bp = new BlockPos(x, y, z);
				...
				if ((world.getBlockState(new BlockPos(x, y, z))).getBlock() == WildasideModBlocks.BALD_CYPRESS_WOOD.get()) {
					{
						BlockPos _bp = new BlockPos(x, y, z);
